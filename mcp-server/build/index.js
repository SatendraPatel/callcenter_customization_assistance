...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        if (name === "get_guide") {
            const guideId = args?.guide_id;
            const search = args?.search;
            let targetGuideId = guideId;
            // If search term provided, find matching guide
            if (search && !guideId) {
                const searchLower = search.toLowerCase();
                const matchingGuide = guidesIndex.guides.find((g) => g.keywords.some((k) => searchLower.includes(k.toLowerCase())) ||
        ...
        if (name === "search_guides") {
            ...
            const matchingGuides = guidesIndex.guides.filter((g) => g.keywords.some((k) => k.toLowerCase().includes(query)) ||
